...
La primera tarea que tuve que realizar fue enterarme de qué hacía el pequeño repositorio que me proporcionaron la primera semana en la empresa. Otros miembros del equipo habían preparado hace varios meses un pequeño prototipo del sistema para una demo, y ese era mi punto de partida. 

Dediqué mis primeros días a leer, ejecutar y tratar de entender el código (además de completar el procedimiento de \textit{onboarding} de la empresa). Me di cuenta de que se podía usar una multitud de modelos, y decidí evaluarlos para ver cuáles usar en la versión definitiva.

\subsubsection{\textit{Papers} (3 días)}\label{subsubsec:papers}
...
El primer dataset que utilicé aparece en multitud de \textit{papers}: UKBENCH \cite{ukb}. Lo descargué de una web de archivo\footnote{https://archive.org/details/ukbench} porque la fuente original ya no está disponible. Está compuesto de 2550 grupos de 4 imágenes, que son fotografías de objetos cotidianos tomadas desde ángulos ligeramente distintos.
...
En ambos datasets evalué los modelos que se listaban en el repositorio inicial. Para cada modelo, para cada imagen, buscaba las top-k más similares en la BBDD (todo el dataset) y el modelo debía encontrar  y marcar como duplicadas las otras imágenes del grupo. En cuanto a las métricas, calculábamos el porcentaje de veces que el modelo marca como duplicadas exactamente las imágenes relevantes, ni más ni menos. También empleábamos Recall@K para ver si el modelo encontraba siquiera las instancias relevantes en su top-k.
...
Al evaluar en el dataset de la temática específica, los resultados eran parecidos. Alexnet y CLIP eran los claros ganadores. Fue durante esta fase cuando me planteé buscar un modelo "más \textit{SOTA}" para ver si aportaba mejor rendimiento, y fue aquí donde encontré Siglip2. Siglip2 resultó ser más lento aún que CLIP, pero también presentaba métricas superiores. 

\subsubsection{Soluciones alternativas (4 días)}\label{subsubsec:alt_solutions}

Mi equipo había desarrollado un prototipo alternativo en el pasado (también basada en \textit{deep learning}) para aquella demo. También evaluamos esta opción en los mismos datasets, pero resultó ser inferior y menos escalable, por lo que terminamos descartándola.

Además, durante mi lectura de \textit{papers} inicial \ref{subsubsec:papers} había aprendido sobre otras técnicas sin \textit{deep learning}. Algunos sistemas empleaban \textit{hashing} u FFT, y presentaban resultados muy competentes a velocidades récord. Lo que nos hizo descartar estos métodos fue que solo sirven para duplicados exactos. En nuestro caso de uso sabríamos que habría duplicados parciales (ej.: fotos de un objeto desde ángulos distintos), y ahí estas técnicas dejarían de funcionar.
