\begin{titlepage}
\vspace*{-35pt}
\begin{minipage}{.15\linewidth}
    \begin{flushleft}
    \hspace*{-15pt}
      %\includegraphics[scale=0.15]{04_imaxes/MUEI.png}
    \end{flushleft}
  \end{minipage}
  \hfill
  \begin{minipage}{.15\linewidth}
    \begin{flushright}
     %\includegraphics[scale=0.10]{04_imaxes/euro_inf_master_verde.png}
     %\includegraphics[scale=0.06]{04_imaxes/euro_inf_seal_bachelor_light background}     
    \end{flushright} 
  \end{minipage}
 
 \vspace*{30pt}
 
\includegraphics[scale=0.30]{04_imaxes/udc}\\[10pt]
  \hspace*{18pt}\textcolor{udcpink}{{\fontencoding{T1}\fontfamily{phv}\selectfont Facultad de Informática}}\\[5pt]
  \hspace*{18pt}\textcolor{udcgray}{{\fontencoding{T1}\fontfamily{phv}\selectfont Grado en Inteligencia Artificial}}\\[5pt]
  
\textcolor{udcpink}{\titlerule[0.5mm]}\\  

  \begin{center}
    {\large PRÁCTICAS EN EMPRESA} \\[25pt]
    \begin{huge}
      \begin{spacing}{1.3}
      \vspace*{10pt}
        \bfseries \titulo
      \end{spacing}
    \end{huge}
  \end{center}
  
\textcolor{udcpink}{\titlerule[0.5mm]}\\[5pt]
  \vfill
  
  \begin{flushright}
    {\large
    \begin{tabular}{ll}
      {\bf Estudiante:} & \alumno \\
      {\bf Tutorización en la empresa:} & \directorA \\
      {\bf Tutorización académica:}     & \directorB \\  % duplicar está línea con los 
                                        % que sean necesarios cambiado la letra
                                        & \departamento
    \end{tabular}}
    
  \end{flushright}
  \rightline{A Coruña, \fecha.}
  
  
\end{titlepage}

\paginablanca